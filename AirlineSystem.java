...
final public class AirlineSystem implements AirlineInterface {
  ...
  public Set<ArrayList<Route>> shortestDistanceItinerary(String source,
    ...
    if(!G.marked[d]) return ret;
      ...
      {
        int v = path.pop();
        lis.add(new Route(cityNames[prevVertex], cityNames[v], G.distTo[v] - G.distTo[prevVertex] , 0.00));
        prevVertex = v;
      }
      ret.add(lis);
      return ret;
  ...
  public boolean addRoute(String source, String destination, int distance,
    ...
    try{
      Route hold = new Route(source, destination, distance, price);
      for (int i = 0; i < G.v; i++) {
        for (Route e : G.adj(i)) {
          if(hold.source == source && hold.destination == destination) return false;
          }
        }
  ...
  public boolean updateRoute(String source, String destination, int distance,
    ...
    for (int i = 0; i < G.v; i++) {
        for (Route e : G.adj(i)) {
          if(hold.source == source && hold.destination == destination)
          {
            e.source = source;
